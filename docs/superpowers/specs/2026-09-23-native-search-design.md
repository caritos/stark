...
3. **Matching fields:** title, notes, and location (events) / title and
   notes (reminders) — the natural things people search by. Case-insensitive
   substring match, not fuzzy/ranked search.
...
- Filtering: for each `store.events`/`store.reminders` entry, a
  ...
  (events) or title + notes (reminders). Empty search text shows no
  results (not the full list) — searching, not browsing, is the point of
  this screen.
